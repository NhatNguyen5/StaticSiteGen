...
#This is **text** with an _italic_ word and a `code block` and an ![obi wan image](https://i.imgur.com/fJRm4Vk.jpeg) and a [link](https://boot.dev)
    def test_split_node_link(self):
        ...
        old_nodes_cases = [
            ...
            [
                TextNode("This is **text** with an _italic_ word and a `code block` and an ![obi wan image](https://i.imgur.com/fJRm4Vk.jpeg) and a [link](https://boot.dev) with trailing text", TextType.TEXT),
            ],
            [
                TextNode("This is **text** with an _italic_ word and a `code block` and an ![obi wan image](https://i.imgur.com/fJRm4Vk.jpeg) and a [link](https://boot.dev)", TextType.TEXT),
                TextNode("This is **text** with an _italic_ word and a `code block` and an ![obi wan image](https://i.imgur.com/fJRm4Vk.jpeg) and a [link](https://boot.dev) with trailing text", TextType.TEXT),
        ...
        for old_nodes, expected_result in zip(old_nodes_cases, expected_results):
            ...
            if expected_result == "Error":
                with self.assertRaises(ValueError, msg=f"Case {case_number} failed"):
                    match = text_to_textnodes(old_nodes)
            else:
                match = text_to_textnodes(old_nodes)
                self.assertEqual(match, expected_result, msg=f"Case {case_number} failed")
